...
int Help_Main()
{
	...
	"     also this disablex exstra safe checks for autoscanner/exported\n"
	...
	" dat - unknown file type (will be autoscanned)\n"
	" tim - PSX texture file\n"
	" adpcm - raw IMA ADPCM, voice data\n"
	" wav - PCM Riff Wave files\n"
	" raw - raw image files (tiles, sprites)\n"
	"\n"
	"=== APPENDIX II: raw image types ===\n"
	" raw - special type for debugging, should have width, height, \n"
	"       bytes offset and color bytes. Well suited for 'data massaging'\n"
	" 1A - multiobject with shared palette, very few files use it\n"
	" 1 - item cards, special case of 1A\n"
	" 2 - multiobject with per-object palette, used for UI graphics\n"
	"\n");
	return 0;
}
