...
int Help_Main()
{
	...
	"  OPTIONAL SWITCHES:\n"
	"    -hashasnames: use hashed names instead of trying to unhash them to normal filenames\n"
	...
	"    -exportcsv filename: export CSV listfile which could be used by Wheel Of Doom\n"
	"\n"
	...
	"\n"
	"-bigfile [bigfilename] [-klist file] -extract filename outfile [-f format] [optional_switches]\n"
	"  extract a specific entry from bigfile\n"
	"  PARMS:\n"
	"   filename: #hash or filename inside pill.big\n"
	"   outfile: output file\n"
	"   format: the format what output file you want, if not set, it will be picked from\n" 
	"           outfile extension. If outfile has no extension, format will be determined\n"
	"			automatically by autoscanner. A 'raw' format is supported on all filetypes\n"
